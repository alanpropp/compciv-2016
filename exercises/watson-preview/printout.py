...
htmlfile.write("<h1>I'm doing my homework Dan! Sincerely, Alan Propp</h1>")
...
for jsonname in glob(join(RECOG_DIR, '*.json')):
    ...
    htmlfile.write("<ol>")
    for classifier_id in img['scores']:
        name = classifier_id['name']
        score = classifier_id['score']
        htmlfile.write('<li>' + name + ' -- ' + str(score) + '</li>')
    htmlfile.write("</ol>")
